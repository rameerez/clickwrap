...
# tables, columns, and indexes — rendered to plain Ruby for the dummy app's test
# database. It is run by `rake db:migrate:reset` across all three CI database
# legs (sqlite / postgres / mysql), so the private helpers at the bottom are
# kept IDENTICAL to the template's: they branch on the connection adapter to emit
# jsonb-vs-json, MySQL's no-default-on-JSON caveat, and MEDIUMTEXT for document
# bodies.
#
# KEEP IN SYNC with the ERB template. test/generators/install_generator_test.rb
# carries a drift test that compares the two column sets and fails the build the
  ...
  def change
    ...
    create_table :clickwrap_events, id: :string, limit: 26 do |t|
      ...
      t.references :presentation, null: true, type: foreign_key_type, index: false,
                                  foreign_key: { to_table: :clickwrap_presentations }
    ...
    create_table :clickwrap_request_evidence, id: primary_key_type do |t|
      ...
      t.decimal :ip_geolocation_latitude, precision: 10, scale: 6
      t.decimal :ip_geolocation_longitude, precision: 10, scale: 6
